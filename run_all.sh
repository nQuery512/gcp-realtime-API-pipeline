...
echo "Chargement de l'environement"
...
## Test d'existence du fichier
[ -f ./data/tmp_data.json ] && echo "Données temporaires déjà présentes" || curl $EXTERNAL_API_URL/?%24%24app_token=$EXTERNAL_API_KEY > ./data/tmp_data.json

echo "Chargement de la table temporaire..."
bq load --source-format=JSON $PROJECT:$DATABASE_NAME.$TMP_TABLE_NAME ./data/tmp_data.json ./data/schema/car_traffic_schema.json

echo "Alimentation de la table historique"
